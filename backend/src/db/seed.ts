...
const slotData: Array<{ providerId: string; date: string; time: string; duration: number }> = [
  // Today
...
for (const s of slotData) {
  const id = uuidv4();
  slotIds.push(id);
  db.prepare('INSERT INTO slots (id, provider_id, date, start_time, duration_minutes, created_by) VALUES (?, ?, ?, ?, ?, ?)').run(
    id, s.providerId, s.date, s.time, s.duration, frontDeskId
  );
}
...
function createAppointment(
  ...
  const id = uuidv4();
  db.prepare('INSERT INTO appointments (id, slot_id, patient_name, patient_email, patient_phone, status) VALUES (?, ?, ?, ?, ?, ?)').run(
    id, slotId, patientName, email, phone, 'requested'
  );
  db.prepare('INSERT INTO audit_log (id, appointment_id, event_type, actor_id, old_value, new_value) VALUES (?, ?, ?, ?, ?, ?)').run(
    uuidv4(), id, 'status_change', frontDeskId, null, 'requested'
  );

  if (finalStatus === 'requested') return id;
  ...
  if (['confirmed', 'checked_in', 'completed', 'no_show', 'cancelled'].includes(finalStatus)) {
    ...
    db.prepare('INSERT INTO audit_log (id, appointment_id, event_type, actor_id, old_value, new_value) VALUES (?, ?, ?, ?, ?, ?)').run(
      uuidv4(), id, 'status_change', frontDeskId, 'requested', 'confirmed'
    );
  }

  if (finalStatus === 'cancelled') {
    db.prepare("UPDATE appointments SET status = 'cancelled', cancellation_reason = ?, updated_at = datetime('now') WHERE id = ?").run(
      cancellationReason || 'Patient request', id
    );
    db.prepare('INSERT INTO audit_log (id, appointment_id, event_type, actor_id, old_value, new_value, metadata) VALUES (?, ?, ?, ?, ?, ?, ?)').run(
      uuidv4(), id, 'status_change', frontDeskId, 'confirmed', 'cancelled', JSON.stringify({ reason: cancellationReason || 'Patient request' })
    );
    return id;
  }

  if (['checked_in', 'completed', 'no_show'].includes(finalStatus)) {
    db.prepare("UPDATE appointments SET status = 'checked_in', updated_at = datetime('now') WHERE id = ?").run(id);
    db.prepare('INSERT INTO audit_log (id, appointment_id, event_type, actor_id, old_value, new_value) VALUES (?, ?, ?, ?, ?, ?)').run(
      uuidv4(), id, 'status_change', frontDeskId, 'confirmed', 'checked_in'
    );
  }

  if (finalStatus === 'no_show') {
    db.prepare("UPDATE appointments SET status = 'no_show', updated_at = datetime('now') WHERE id = ?").run(id);
    db.prepare('INSERT INTO audit_log (id, appointment_id, event_type, actor_id, old_value, new_value) VALUES (?, ?, ?, ?, ?, ?)').run(
      uuidv4(), id, 'status_change', frontDeskId, 'checked_in', 'no_show'
    );
  }
  ...
  if (finalStatus === 'completed') {
    ...
    db.prepare('INSERT INTO audit_log (id, appointment_id, event_type, actor_id, old_value, new_value) VALUES (?, ?, ?, ?, ?, ?)').run(
      uuidv4(), id, 'status_change', frontDeskId, 'checked_in', 'completed'
    );
  }
...
db.prepare('INSERT INTO visit_notes (id, appointment_id, provider_id, content) VALUES (?, ?, ?, ?)').run(
  uuidv4(), todayApptIds[0], provider1Id,
  'Patient presented with right shoulder pain. ROM limited to 120 degrees abduction. Prescribed stretching exercises and heat therapy. Follow-up in 2 weeks.'
);
db.prepare('INSERT INTO audit_log (id, appointment_id, event_type, actor_id, new_value) VALUES (?, ?, ?, ?, ?)').run(
  uuidv4(), todayApptIds[0], 'visit_note_added', provider1Id, 'Patient presented with right shoulder pain...'
);

// Add care team member to one of today's appointments
db.prepare('INSERT INTO care_team (appointment_id, provider_id, added_by) VALUES (?, ?, ?)').run(
  todayApptIds[2], provider2Id, frontDeskId
);
db.prepare('INSERT INTO audit_log (id, appointment_id, event_type, actor_id, new_value) VALUES (?, ?, ?, ?, ?)').run(
  uuidv4(), todayApptIds[2], 'care_team_added', frontDeskId, provider2Id
);

// --- Tomorrow's appointments ---
const tomorrowApptIds: string[] = [];
